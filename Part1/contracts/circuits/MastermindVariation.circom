...
// Mastermind for Kids was choosen
...
template MastermindVariation() {
    ...
    // Create a constraint that the solution and guess themes are all less than 6 since each number from 0 to 6 [0,1,2,3,4,5] defines a different theme 
    for (j=0; j<3; j++) {
        lessThan[j] = LessThan(3);
        lessThan[j].in[0] <== guess[j];
        lessThan[j].in[1] <== 6;
        lessThan[j].out === 1;
        lessThan[j+3] = LessThan(3);
        lessThan[j+3].in[0] <== soln[j];
        lessThan[j+3].in[1] <== 6;
        lessThan[j+3].out === 1;
        for (k=j+1; k<3; k++) {
            // Create a constraint that the solution and guess themes are unique. no duplication.
